...
(function ($) {
  ...
  */
  ...
  $('.word').wrap('<span></span>');
  $('[data-app-overlay]').append('<div class="app-overlay"></div>');
  ...
  /**
   Header Menu Button
   **/
  $('.app-menu-btn').on('click', function () {
    var obj = $(this);
    if (obj.hasClass('btn--active')) {
      $('body').removeClass('app--noscroll');
      $('.app-header').removeClass('header--active');
      obj.removeClass('btn--active');
      obj.addClass('app--notouch');
      obj.parent().find('.app-menu-popup').removeClass('menu--ready');
      obj.parent().find('.app-menu-container').addClass('app--noscroll');
      setTimeout(function () {
        obj.parent().find('.app-menu-popup').removeClass('menu--open');
      }, 300);
      setTimeout(function () {
        obj.removeClass('app--notouch');
        obj.parent().find('.app-menu-popup').removeClass('menu--visible');
      }, 1600);
    } else {
      $('body').addClass('app--noscroll');
      $('.app-header').addClass('header--active');
      obj.addClass('btn--active');
      obj.addClass('app--notouch');
      obj.parent().find('.app-menu-popup').addClass('menu--visible');
      obj.parent().find('.app-menu-popup').addClass('menu--open');
      setTimeout(function () {
        obj.removeClass('app--notouch');
        obj.parent().find('.app-menu-container').removeClass('app--noscroll');
        obj.parent().find('.app-menu-popup').addClass('menu--ready');
      }, 600);
    }
    return false;
  });
  ...
  */
  $('.app-menu').on('click', 'a', function () {
    if (!$(this).parent().hasClass('menu-item-has-children')) {
      $('.app-header .app-menu-btn.btn--active').trigger('click');
    }
  });
  ...
  */
  $('.app-menu').on('click', '.menu-item-has-children > .icon, .dropdown-link > a', function () {
  ...
  */
  if ($('.footer--fixed').length) {
    var footer_fixed = $('.footer--fixed .app-footer').height();
  ...
  /*
    Intro Style
  */
  if ($('.app-intro.intro--black').length) {
  ...
  */
  $('.js-hero-parallax').each(function () {
    ...
    heroParallax.pagepiling({
      ...
      sectionSelector: '.app-hero-parallax-section',
      afterRender: function () {
        if (heroParallax.find('.app-hero-parallax-section.active').hasClass('section--dark')) {
          setDark();
        }
      ...
      afterLoad: function (anchorLink, index) {
        var current = heroParallax.find('.app-hero-parallax-section.active');
  ...
  function setDark() {
    $('.app-page').addClass('parallax--dark');
    $('.app-header').removeClass('header--white');
    $('.app-hero-parallax .app-prev').removeClass('nav--white');
    $('.app-hero-parallax .app-next').removeClass('nav--white');
  }

  function removeDark() {
    $('.app-page').removeClass('parallax--dark');
    $('.app-header').addClass('header--white');
    $('.app-hero-parallax .app-prev').addClass('nav--white');
    $('.app-hero-parallax .app-next').addClass('nav--white');
  }
  ...
  */
  $('.js-hero-slider').each(function () {
    ...
    heroMainSwiper.find('.app-paginations-container').append('<div class="swiper-nav-active"></div>');
      ...
      on: {
        progress: function () {
          ...
          for (var i = 0; i < swiper.slides.length; i++) {
            var slideProgress = swiper.slides[i].progress, innerOffset = swiper.width * 0.5,
  ...
  */
  $('.js-blog-carousel').each(function () {
    ...
    blogSwiper.parent().find('.app-paginations-container').append('<div class="swiper-nav-active"></div>');
  ...
  */
  $('.js-history-slider').each(function () {
    ...
    hisSwiper.find('.app-paginations-container').append('<div class="swiper-nav-active"></div>');
  ...
  */
  $('.app-play-btn').on('click', function () {
  ...
  /*
    - Services Hover
  */
  $('.app-service-grid-item.active--default').each(function () {
    $(this).closest('.app-services-grid-fw').hover(function () {
      if ($(this).find('.app-service-grid-item').hasClass('active--default')) {
        $(this).find('.app-service-grid-item').removeClass('active');
      }
    }, function () {
      if ($(this).find('.app-service-grid-item').hasClass('active--default')) {
        $(this).find('.app-service-grid-item.active--default').addClass('active');
      }
    });
  });
  ...
  /*
    - Number Text
  */
  $('.app-circle-text .label').each(function () {
  ...
  */
  if ($('.app-showcase-items').length) {
    $('.app-showcase-items').find('.app-showcase-item:first-child').addClass('hover');
  }
  $('.app-showcase-item').mouseenter(function () {
    var obj = $(this);
    obj.closest('.app-showcase-items').find('.app-showcase-item').removeClass('hover');
    obj.addClass('hover');
  });
  ...
  */
  if ($(window).width() > 767) {
    if ($('.app-services-showcase').length) {
      $('.app-services-showcase').find('.items ul li:first-child').addClass('hover');
    }
    $('.app-services-showcase .items ul li .title').mouseenter(function () {
  ...
  /**
   - Video
   **/
  $('.app-video').on('click', '.play, .image', function () {
    $(this).closest('.app-video').addClass('active');
    var iframe = $(this).closest('.app-video').find('.js-video-iframe');
  ...
  /*
    - Button
  */
  $('button.app-btn.app-hover-btn').each(function () {
  ...
  */
  $('.pager').each(function () {
    ...
    btn_next.html('<i></i>');
    btn_prev.html('<i></i>');
    btn_next.addClass('app-next app-hover-2');
    btn_prev.addClass('app-prev app-hover-2');
  });

  /*
    - Portfolio
  */
  var $container = $('.app-portfolio-items');
  ...
  /*
    - Filter
  */
  $('.app-filter-nav-active').each(function () {
    $(this).css({'width': $(this).closest('.app-filter.filter--default').find('.item--active').parent().width() + 6 + 'px'});
  });
  $('.app-filter').on('click', 'button', function () {
    var filterValue = $(this).attr('data-filter');
    $container.isotope({filter: filterValue});
    $(filterValue).find('.image').attr('data-scroll', 'in');
    $(filterValue).find('.splitting').attr('data-scroll', 'in');

    $('.app-filter button').removeClass('item--active');
    $(this).addClass('item--active');

    var current_pos = $(this).closest('.app-filter.filter--default').find('.item--active').parent().position().left;
    var current_width = $(this).closest('.app-filter.filter--default').find('.item--active').parent().width();
    $(this).closest('.app-filter.filter--default').find('.app-filter-nav-active').css({'width': current_width + 6 + 'px'});
    $(this).closest('.app-filter.filter--default').find('.app-filter-nav-active').css({'left': current_pos - 3 + 'px'});
  });

  /**
   Collapse
   **/
  $('.app-collapse-item').on('click', '.app-collapse-btn', function () {
    if ($(this).closest('.app-collapse-item').hasClass('opened')) {
      $(this).closest('.app-collapse-item').removeClass('opened');
      $(this).removeClass('active');
      $(this).next().slideUp();
    } else {
      $(this).closest('.app-collapse-item').addClass('opened');
      $(this).addClass('active');
      $(this).next().slideDown();
    }
  });
