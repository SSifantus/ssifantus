import {useEffect} from "react";
import Data from "@data/sections/services-3.json";
import {servShowcaseHover} from "@common/utilits";

const Services3Section = ({services}) => {
  useEffect(() => {
    servShowcaseHover();
  }, []);

  return (
    <>
      <section className="app-section app-section-bg gap-top-140 gap-bottom-140">
        <div className="container-md">
          <div className="row">
            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-7">
              <div className="app-heading gap-bottom-60">
                <div className="app-subtitle-1">
                  <span>{Data.subtitle}</span>
                </div>
                <h2 className="app-title-2">
                  <span dangerouslySetInnerHTML={{__html: Data.title}}/>
                </h2>
                <div className="app-text">
                  <p dangerouslySetInnerHTML={{__html: Data.text}}/>
                </div>
              </div>
            </div>
            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-4 offset-lg-1 hide-on-mobile">

            </div>
          </div>

          <div className="app-services-showcase">
            <div className="row">
              <div className="col-xs-0 col-sm-0 col-md-0 col-lg-8"></div>
              <div className="col-xs-12 col-sm-12 col-md-12 col-lg-4">
                <div className="app-subtitle-1">
                  <span>{Data.title2}</span>
                </div>
                <div className="items">
                  <span className="img-circle app-circle-move"/>
                  <ul>
                    {services.slice(0, Data.numOfItems).map((item, key) => (
                      <li key={`services3-item-${key}`}>
                        <div className="image">
                          <span className="img" style={{"backgroundImage": "url(" + item.image + ")"}}/>
                        </div>
                        <h5 className="title">
                          <a href={`/services/${item.id}`}>
                            <span className="name">
                              <span className="app-lnk">
                                <span data-splitting data-app-scroll>{item.title}</span>
                              </span>
                            </span>
                          </a>
                        </h5>
                      </li>
                    ))}
                  </ul>
                </div>
                <a href={Data.button.link} className="app-btn app-hover-btn btn--border">
                  <span>{Data.button.label}</span>
                </a>
              </div>
            </div>
          </div>

        </div>
      </section>
    </>
  );
};

export default Services3Section;
